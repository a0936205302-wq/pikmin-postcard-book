...
class _DetailPageState extends State<DetailPage> {
  ...
  Future<void> _saveEditing() async {
    ...
    if (_editingCategory == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('請選擇菇點或花點')));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('已更新明信片')));
    } catch (error) {
      if (!mounted) {
        return;
      }
  ...
  Future<void> _toggleOwned(bool value) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('更新擁有狀態失敗：$error')));
    } finally {
      if (mounted) {
        setState(() {
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: () async {
                        ...
                        ScaffoldMessenger.of(
                          context,
                        ).showSnackBar(const SnackBar(content: Text('已複製座標')));
                      },
